...
use App\Http\Requests\BrandBioUpdateRequest;
...
class BrandsApiController extends Controller
{
    ...
    public function show(GetBrandByIDRequest $request)
    {
        $criteria = new Criteria($request);
        $result = $this->brandService->getBrandByID($criteria);
        return response()->json($result->nullCheckResp(), $result->getHttpStatus());
    }
    ...
    public function updateBio(BrandBioUpdateRequest $request)
    {
        DB::beginTransaction();
        $result = $this->brandService->updateBio(new Criteria($request));
        $result->completeTransaction();
        return response()->json($result->nullCheckResp(), $result->getHttpStatus());
    }
}
